...
export function DynamicWaves() {
  ...
  const drawDynamicWaves = useCallback((ctx: CanvasRenderingContext2D, time: number) => {
    ...
    for (let layer = 0; layer < 5; layer++) {
      ...
      const color = layer === 0 ? '#1DB954' :
        layer === 1 ? '#1ed760' :
          layer === 2 ? '#91e9b3' :
            layer === 3 ? '#b5f0ce' : '#ffffff';
      const opacity = 0.8 - layer * 0.15;

      const waveFunction = (x: number): number => {
        return (
          Math.sin(x * frequency + time * layerSpeed) * amplitude +
          Math.sin(x * frequency * 2.3 + time * layerSpeed * 1.7) * (amplitude * 0.6) +
          Math.sin(x * frequency * 3.1 + time * layerSpeed * 0.9) * (amplitude * 0.3) +
